    ...
    A:
        ...
        (c) project/Gemfile：is a list of all gems that you want to include in the project. It is used with bundler (also a gem) to install, update, remove gems, related gems, and their version, bundle result will be loged in Gemfile.lock file。
    ...
    (c) C：Controller, it directs traffic by querying the models for specific data, and organizing that data (searching, sorting, messaging it) into a form that fits the needs of a given view.
    ...
    A:
    (a) Model file：is the M in MVC , is the Active Record  which is the layer of the system responsible for representing business data and logic.
    Ex: class User < ActiveRecord::Base
          has_many :post1s   
        end
    (b) Migration file：is used by Active Record  to evolve your database schema over time (need to run rake db:migrate to create the table in database), instead of writing schema in pure SQL.
    ...
    A:
    Active Record is the M(the model) in MVC, it is an implementation of the "Active Record pattern" which itself is a description of an Object Relational Mapping system.
